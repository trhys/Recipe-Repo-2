...
async function printList() {
	...
	try {
		const url = `/shoppinglists/printlist`
		const body = {
			method: 'POST',
			headers: {
				'Authorization': `Bearer ${token}`,
			...
			body: JSON.stringify({id:document.getElementById('print-shopping-list').value}),
		}
		let response = await fetch(url, body);
		...
		if (response.ok) {
			...
			document.write(newHTML);
			document.close();
			window.history.pushState({}, '', `/shoppinglists/printlist`);
			return
		}
